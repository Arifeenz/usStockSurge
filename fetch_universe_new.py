...
def calc_indicators(df: pd.DataFrame) -> dict:
    ...
    liquidity_score  = int(vol_ratio_v >= 1.0) + int(rvol_v >= 1.0)
    momentum_score   = int(roc_v >= 0) + int(macd_v >= 0) + int(accel_v >= 0)
    trend_score_new  = int(trend_v >= 1) + int(close_v >= ema50_v)
    bounce_zone      = (int(-0.08 <= dist_v <= 0.05)
                      + int(40 <= rsi_v <= 70)
                      + int(0.15 <= bb_pos_v <= 0.70))
    risk_control     = int(close_v >= 1.0) + int(stoch_v <= 85)
...
def run(args):
    ...
    # ── อัปเดต score_history.json ──────────────────────────────
    # เก็บ bounce_score รายวันของแต่ละหุ้น ไว้ดู trend ใน Watchlist
    history_path = os.path.join(os.path.dirname(os.path.abspath(out_path)), "score_history.json")
    KEEP_DAYS = 30  # เก็บย้อนหลัง 30 วัน (เว็บแสดงแค่ 7 วัน)
    ...
    today = results[0].get("date", "") if results else ""
    if not today:
        import datetime
    ...
    for row in results:
        sym   = row.get("symbol")
        score = row.get("bounce_score")
        if not sym or score is None:
            continue
        if sym not in history:
            history[sym] = []
        if history[sym] and history[sym][-1]["date"] == today:
            history[sym][-1]["score"] = score  # re-run วันเดียวกัน → อัปเดตแทน
        else:
            history[sym].append({"date": today, "score": score})
        history[sym] = history[sym][-KEEP_DAYS:]

    with open(history_path, "w", encoding="utf-8") as f:
        json.dump(history, f, ensure_ascii=False)

    print(f"📈 อัปเดต score history → {history_path}  ({len(history)} symbols)")
